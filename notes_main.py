...
list_notes_label = QLabel('Список значния')
...
list_tags_label = QLabel('Списки тегов')
list_tags = QListWidget()
...
def add_note():
    note_name, ok = QInputDialog.getText(notes_win, "Добавить заметку", "Нзвание заметки")
    print(ok)
    if ok and note_name != "":
...
def del_tag():
    if list_notes.selectItems():    
        key = list_notes.selectItems()[0].text()
        tag = list_tags.selectitems()[0].text()
...
def search_tag():
    ...
    if button_tag_search.text() == "Искать заметки по тегу" and tag:
        ...
        field_notes.clear()
        list_tags.clear()
        list_notes_search.setText("Искать заметки по тегу")
        print(button_tag_search.text())
